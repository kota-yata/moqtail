...
describe('utils/bytes', () => {
  ...
  test('serializeQuicVarInt serializes correctly', () => {
    ...
    expect(() => serializeQuicVarInt(-1 as any)).toThrow();
  });
  ...
  test('multiple combinations of stringToVarBytes and varBytesToStringFromArray serialize/deserialize correctly', () => {
    ...
    const deserialized3 = varBytesToStringFromArray(
      serialized,
      deserialized1.byteLength + deserialized2.byteLength
    );
    expect(deserialized3.value).toEqual(originalStrs[2]);
    const deserialized4 = varBytesToStringFromArray(
      serialized,
      deserialized1.byteLength + deserialized2.byteLength + deserialized3.byteLength
    );
