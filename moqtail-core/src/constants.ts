export const MOQT_DRAFT08_VERSION = 0xff000008;
export const MOQT_DRAFT09_VERSION = 0xff000009;
export const MOQT_DRAFT10_VERSION = 0xff00000A;
export const MOQT_DRAFT11_VERSION = 0xff00000B;
export const MOQT_SUPPORTED_VERSIONS = [MOQT_DRAFT10_VERSION, MOQT_DRAFT11_VERSION];

export const PARAMETER = {
  AUTHORIZATION_TOKEN: { KEY: 0x01 },
  DELIVERY_TIMEOUT: { KEY: 0x02 },
  MAX_CACHE_DURATION: { KEY: 0x04 },
  SETUP: {
    PATH: { KEY: 0x01 },
    MAX_REQUEST_ID: { KEY: 0x02 },
    MAX_AUTH_TOKEN_CACHE_SIZE: { KEY: 0x04 }
  }
} as const;
export type PARAMETER = ObjectValueList<typeof PARAMETER>;

export const CONTROL_MESSAGE = {
  CLIENT_SETUP: 0x20,
  SERVER_SETUP: 0x21,
  GOAWAY: 0x10,
  MAX_REQUEST_ID: 0x15,
  REQUESTS_BLOCKED: 0x1A,
  SUBSCRIBE: 0x3,
  SUBSCRIBE_OK: 0x4,
  SUBSCRIBE_ERROR: 0x5,
  UNSUBSCRIBE: 0xA,
  SUBSCRIBE_UPDATE:  0x2,
  SUBSCRIBE_DONE: 0xB,
  FETCH: 0x16,
  FETCH_OK: 0x18,
  FETCH_ERROR: 0x19,
  FETCH_CANCEL: 0x17,
  TRACK_STATUS_REQUEST: 0xD,
  TRACK_STATUS: 0xE,
  ANNOUNCE: 0x6,
  ANNOUNCE_OK: 0x7,
  ANNOUNCE_ERROR: 0x8,
  UNANNOUNCE: 0x9,
  ANNOUNCE_CANCEL: 0xC,
  SUBSCRIBE_ANNOUNCES: 0x11,
  SUBSCRIBE_ANNOUNCES_OK: 0x12,
  SUBSCRIBE_ANNOUNCES_ERROR: 0x13,
  UNSUBSCRIBE_ANNOUNCES: 0x14,
} as const;

export const SESSION_CLOSE_ERROR_REASON = {
  NO_ERROR: 0x0,
  INTERNAL_ERROR: 0x1,
  UNAUTHORIZED: 0x2,
  PROTOCOL_VIOLATION: 0x3,
  DUPLICATE_TRACK_ALIAS: 0x4,
  PARAMETER_LENGTH_MISMATCH: 0x5,
  TO_MANY_SUBSCRIBES: 0x6,
  GOAWAY_TIMEOUT: 0x10,
  CONTROL_MESSAGE_TIMEOUT: 0x11,
  DATA_STREAM_TIMEOUT: 0x12,
} as const;

export const SUBSCRIBE_ERROR_REASON = {
  INTERNAL_ERROR: 0x0,
  UNAUTHORIZED: 0x1,
  TIMEOUT: 0x2,
  NOT_SUPPORTED: 0x3,
  TRACK_DOES_NOT_EXIST: 0x4,
  INVALID_RANGE: 0x5,
  RETRY_TRACK_ALIAS: 0x6,
} as const;
export type SUBSCRIBE_ERROR_REASON = ObjectValueList<typeof SUBSCRIBE_ERROR_REASON>;

export const SUBSCRIBE_DONE_REASON = {
  INTERNAL_ERROR: 0x0,
  UNAUTHORIAZED: 0x1,
  TRACK_ENDED: 0x2,
  SUBSCRIPTION_ENDED: 0x3,
  GOING_AWAY: 0x4,
  EXPIRED: 0x5,
  TOO_FAR_BEHIND: 0x6,
} as const;
export type SUBSCRIBE_DONE_REASON = ObjectValueList<typeof SUBSCRIBE_DONE_REASON>;

export const FETCH_ERROR_REASON = {
  INTERNAL_ERROR: 0x0,
  UNAUTHORIZED: 0x1,
  TIMEOUT: 0x2,
  NOT_SUPPORTED: 0x3,
  TRACK_DOES_NOT_EXIST: 0x4,
  INVALID_RANGE: 0x5,
} as const;
export type FETCH_ERROR_REASON = ObjectValueList<typeof FETCH_ERROR_REASON>;

export const ANNOUNCE_ERROR_REASON = {
  INTERNAL_ERROR: 0x0,
  UNAUTHORIZED: 0x1,
  TIMEOUT: 0x2,
  NOT_SUPPORTED: 0x3,
  UNINTERESTED: 0x4,
} as const;
export type ANNOUNCE_ERROR_REASON = ObjectValueList<typeof ANNOUNCE_ERROR_REASON>;

export const SUBSCRIBE_ANNOUNCES_ERROR_REASON = {
  INTERNAL_ERROR: 0x0,
  UNAUTHORIZED: 0x1,
  TIMEOUT: 0x2,
  NOT_SUPPORTED: 0x3,
  NAMESPACE_PREFIX_UNKNOWN: 0x4
} as const;
export type SUBSCRIBE_ANNOUNCES_ERROR_REASON = ObjectValueList<typeof SUBSCRIBE_ANNOUNCES_ERROR_REASON>;

export const STREAM = {
  SUBGROUP_HEADER: 0x4,
  FETCH_HEADER: 0x5
} as const;

export const DATAGRAM = {
  OBJECT_DATAGRAM: 0x1,
  OBJECT_DATAGRAM_STATUS: 0x2
} as const;

export const FETCH_TYPE = {
  STANDALONE: 0x1,
  JOINING: 0x2
} as const;
export type FETCH_TYPE = ObjectValueList<typeof FETCH_TYPE>;

export const GROUP_ORDER = {
  PUBLISHER: 0x0,
  ASCENDING: 0x1,
  DESCENDING: 0x2
} as const;
export type GROUP_ORDER = ObjectValueList<typeof GROUP_ORDER>;

export const OBJECT_STATUS = {
  NORMAL: 0x0,
  OBJECT_DOES_NOT_EXIST: 0x1,
  GROUP_DOES_NOT_EXIST: 0x2,
  END_OF_GROUP: 0x3,
  END_OF_TRACK_AND_GROUP: 0x4,
  END_OF_TRACK: 0x5,
} as const;
export type OBJECT_STATUS = ObjectValueList<typeof OBJECT_STATUS>;

export const TRACK_STATUS_CODE = {
  IN_PROGRESS: 0x0,
  NOT_EXIST: 0x1,
  NOT_BEGUN: 0x2,
  FINISHED: 0x3,
  SENDER_IS_RELAY: 0x4
} as const;
export type TRACK_STATUS_CODE = ObjectValueList<typeof TRACK_STATUS_CODE>;

export const SUBSCRIBE_FILTER = {
  LATEST_OBJECT: 0x2,
  ABSOLUTE_START: 0x3,
  ABSOLUTE_RANGE: 0x4
} as const;
export type SUBSCRIBE_FILTER = ObjectValueList<typeof SUBSCRIBE_FILTER>;

export const CONTENT_EXISTS = {
  NO: 0x0,
  YES: 0x1
} as const;
export type CONTENT_EXISTS = ObjectValueList<typeof CONTENT_EXISTS>;
